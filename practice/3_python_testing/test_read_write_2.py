...
def test_task_read_write_2(tmpdir, mocker):
    mocker.patch('task_read_write_2.generate_words',
                 return_value=['abc', 'def', 'xyz'])
    file_1 = tmpdir.join('file_1.txt')
    file_2 = tmpdir.join('file_2.txt')

    read_write_2(file_1, file_2)

    assert file_1.read_text(encoding='utf-8') == "abc\ndef\nxyz"
    assert file_2.read_text(encoding='cp1252') == "xyz,def,abc"
